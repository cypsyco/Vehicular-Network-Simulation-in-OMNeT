...
void PingApp::sendPingRequest()
{
    ...
    //cout<<"SendSeqNo(ping): "<<sendSeqNo<<endl;
    ...
    //cout<<"From here add sendSeqNo + 1 :"<<sendSeqNo<<endl;
    sendSeqNo++;
    sentCount++;
    //cout<<"Added sendSeqNo :"<<sendSeqNo<<endl;
}
...
void PingApp::countPingResponse(int bytes, long seqNo, simtime_t rtt, bool isDup)
{
    ...
    cout<<"this is expectedReplySeqNo"<<expectedReplySeqNo<<endl;
    if (isDup)
        numDuplicatedPongs++;
    else
        numPongs++;
...
//        cout << "--------------------------------------------------------" << endl;
//
//        cout << "sent: " << sendSeqNo << "   receiveStandardHost6d: " << numPongs << "   loss rate (%): " << (100 * lossCount / (double)sendSeqNo) << endl;
