...
void Ipv6NeighbourDiscovery::handleMessage(cMessage *msg)
{
    if (msg->isSelfMessage()) {
        ...
        std::ofstream file("/home/jyh0872/Downloads/veinsTermProject/examples/veins/results/EndToEndDelay/delay.txt", std::ios::app);
        if (file.is_open()) {
            file << "EndToEndDelay: " << delay << "s\n";
            file.close();
        } else {
            EV_ERROR << "Unable to open file for writing delay data\n";
        }
        ...
        if (msg->getKind() == MK_SEND_PERIODIC_RTRADV) {
            ...
            EV_INFO << "Address Registration Update! Resend NS to update register time!\n";
...
//        throw cRuntimeError("The nceKey is nullptr at nce->MAC=%s, isRouter=%d",
//                nce->macAddress.str().c_str(), nce->isRouter);
       std::cout<<"nceKey is nullptr!delete failed!"<<endl;
   else if (simTime() > nce->registLifeTime &&nceKey!=nullptr) {
        neighbourCache.remove(nceKey->address, nceKey->interfaceID);
...
//        entry->routerExpiryTime = simTime() + ie->ipv6Data()->_getReachableTime();
        ...
        l3AddressInd->setSrcAddress(ie->ipv6Data()->getPreferredAddress());
