...
void Ipv6::routePacket(Packet *packet, const InterfaceEntry *destIE, const InterfaceEntry *fromIE, Ipv6Address requestedNextHopAddress, bool fromHL)
{
    ...
    EV_INFO <<"Unicast packet Dest Address is: "<< destAddress <<endl;
...
//        if (!rt->isRouter() && !(packet->getArrivalGate()->isName("ndIn"))) {
        if (!rt->isRouter() && !rt->isForwarding()&& !(packet->getArrivalGate()->isName("ndIn"))) {
...
void Ipv6::routeMulticastPacket(Packet *packet, const InterfaceEntry *destIE, const InterfaceEntry *fromIE, bool fromHL)
{
    ...
    if (fromIE != nullptr) {
        ...
        if (rt->isLocalAddress(destAddr)) {
            EV_INFO <<"DestAddr "<< destAddr << "is local address" << endl;
        ...
        if (destAddr.getMulticastScope() <= 2) {
            EV_INFO << "Dest address multicast scope is :"<< destAddr.getMulticastScope() <<endl;
            EV_INFO << "multicast dest address is :"<< destAddr <<endl;
...
void Ipv6::sendDatagramToOutput(Packet *packet, const InterfaceEntry *destIE, const MacAddress& macAddr)
{
    ...
    //XIANG ZHONG 19.07.07
    /*
    if (packet->findTag<MacAddressReq>()==nullptr)
        packet->addTagIfAbsent<MacAddressReq>()->setDestAddress(macAddr);

    else if (packet->getTag<MacAddressReq>()->getDestAddress()==MacAddress::UNSPECIFIED_ADDRESS)
        packet->addTagIfAbsent<MacAddressReq>()->setDestAddress(macAddr);

    else if (packet->findTag<MacAddressReq>()!=nullptr &&packet->getTag<MacAddressReq>()->getDestAddress()!=MacAddress::UNSPECIFIED_ADDRESS
    ...
    if (packet->findTag<MacAddressReq>()!=nullptr &&packet->getTag<MacAddressReq>()->getDestAddress()!=MacAddress::UNSPECIFIED_ADDRESS
                && macAddr==MacAddress::BROADCAST_ADDRESS) {
        }
    else
        packet->addTagIfAbsent<MacAddressReq>()->setDestAddress(macAddr);
        ...
        int& interfaceId, Packet *packet, bool fromHL)
{
    ...
    if (interfaceId == -1) {
        ...
        //XIANG ZHONG 19.07.08
        //New routing mechanism, determine next hop
        Neighbour *neighborRoute = nd->neighbourCache.lookupNeighborRoute(destAddress, interfaceId);

        //MA behavior: if the neighborRoute is not null, set vehicle's default RSU as nextHop
        // if the vehicle is directly attached to MA, the relayAddr is set to its own address
        if (neighborRoute!=nullptr) {
            EV_INFO<<"get next hop from neighborRoute"<<endl;
            nextHop = neighborRoute->relayAddr;
        }
        //Vehicle behavior: set default RSU as nextHop
        else if (nd->neighbourCache.getDefaultRouterList().begin() != nd->neighbourCache.getDefaultRouterList().end())
        {
                auto it =nd->neighbourCache.getDefaultRouterList().begin();
                Neighbour& nce = *it;
                nextHop = nce.nceKey->address;
        }
        //RSU behavior: send vehicle's message to MA
        else if (nd->rt6->isRouter()&&!nd->rt6->isHomeAgent()) {
            EV_INFO<<"next hop is MA"<<endl;
            nextHop = Ipv6Address("aaaa:2:65:0:8aa:ff:fe00:9");
            interfaceId = 102;

        }
        //MA behavior: send packets to directly attached CN
        else {
            EV_INFO<<"nextHop is destAddress."<<endl;
            nextHop = destAddress;
            interfaceId = 103;
        }
